...
class SchoolScheduleParser {
  ...
  async getMonthlySchedule(date, recache) {
    ...
    if (recache || !schoolDB[month] || Date.now() - schoolDB[month].lastCache > CACHE_INTERVAL) schoolDB[month] = await parseScheduleData(date, this.schoolLocation, this.schoolCode, this.schoolType);

    return schoolDB[month].schedules;
  }
}

exports.default = (schoolLocation, schoolCode, schoolType) => new SchoolScheduleParser(schoolLocation, schoolCode, schoolType);
...
async function getRawDatabase(date, schoolLocation, schoolCode, schoolType) {
  return await (0, _http_request2.default)(GLOBAL_URL, 'GET', `/${SCHEDULE_MONTH_URL}?domainCode=${schoolLocation}&contEducation=${schoolLocation}&schulCode=${schoolCode}&schulKndScCode=${KNDCODE}&schulCrseScCode=${schoolType}&ay=${date.getFullYear()}&mm=${date.getMonth() + 1}`);
}
